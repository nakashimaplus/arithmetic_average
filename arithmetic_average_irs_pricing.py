...
def main():
    t_0 = 0
    beta = 1  # s_qの中でsigma_normal_negative2を使い、かつbeta=1だとおかしい
    r_0 = 0.05
    ...
    def generate_payoff_function(t_0, t_s, t_e):
        def payoff_function(forward_rate, strike):
            return g_function(t_s=t_s, t_e=t_e, d_n=t_0, x=forward_rate)
        return payoff_function

    payoff_mc = payoff_by_sabr_mc_without_discout(
        t_0, beta, r_0, tau_0, tau_1, strikes, generate_payoff_function(t_0, tau_0, tau_1), rho, nu, alpha, q, num_mc_path, num_grid)-strikes
    ...
    _s_q = s_q(t_0, tau_1, r_0, r_0, alpha_hat, beta, rho_hat, nu_hat)

    payoff_approximation = list()
    for strike in strikes:
        payoff_approximation.append(
            math.log(1+r_0)-strike + 1/2*(-1*(1/(1+r_0))**2)*_s_q**2)
    ...
    plt.plot(strikes, sigma_approximation_solution, label='all effective')
    plt.xlabel("K")
    plt.grid()
    plt.legend()
    plt.show()
